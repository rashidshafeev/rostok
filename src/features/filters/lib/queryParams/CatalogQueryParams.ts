...
export interface CatalogQueryParams extends BaseFilterParams, SortingParams, PaginationParams {}
...
export class CatalogQueryParamsUtil {
  static parseQueryParams(queryString: string): CatalogQueryParams {
    ...
    if (params.has("min_price")) {
      result.min_price = params.get("min_price") === "null"
        ? null
        : parseInt(params.get("min_price")!, 10);
    }
    if (params.has("max_price")) {
      result.max_price = params.get("max_price") === "null"
        ? null
