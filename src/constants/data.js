import chair from '../assets/temp-images/chair.png';
import offer1 from '../assets/temp-images/offer1.jpg';
import offer2 from '../assets/temp-images/offer2.jpg';
import offer3 from '../assets/temp-images/offer3.jpg';
import room1 from '../assets/temp-images/room1.jpg';
import room2 from '../assets/temp-images/room2.jpg';
import room3 from '../assets/temp-images/room3.jpg';
import room4 from '../assets/temp-images/room4.jpg';
import room5 from '../assets/temp-images/room5.jpg';
import room6 from '../assets/temp-images/room6.jpg';
import brand1 from '../assets/temp-images/brand1.png';
import brand2 from '../assets/temp-images/brand2.png';
import brand3 from '../assets/temp-images/brand3.png';
import brand4 from '../assets/temp-images/brand4.png';
import brand5 from '../assets/temp-images/brand5.png';
import brand6 from '../assets/temp-images/brand6.png';
import adv1 from '../assets/icons/adv1.svg';
import adv2 from '../assets/icons/adv2.svg';
import adv3 from '../assets/icons/adv3.svg';
import adv4 from '../assets/icons/adv4.svg';
import adv5 from '../assets/icons/adv5.svg';
import adv6 from '../assets/icons/adv6.svg';

export const bannerOne = [
  {
    id: 1,
    title: 'Осенние скидки до 50%',
    desc: 'Успейте сделать свой дом еще уютнее и стильнее без лишних затрат',
    img: chair,
  },
  {
    id: 2,
    title: 'Lorem ipsum dolor sit amet.',
    desc: 'Lorem ipsum dolor sit amet consectetur adipisicing elit debitis, quis.',
    img: chair,
  },
  {
    id: 3,
    title: 'Осенние скидки до 50% 3',
    desc: 'Успейте сделать свой дом еще уютнее и стильнее без лишних затрат',
    img: chair,
  },
];

export const products = [
  {
    id: 1,
    files: [
      {
        medium:
          'https://podvesnov.ru/wp-content/uploads/2020/10/rotang-uley-01.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'Подвесное кресло 2М бежевый/черный',
    article: '127504',
    type: 'hit',
    cost: '32 999',
  },
  {
    id: 2,
    files: [
      {
        medium:
          'https://specmebel-tver.ru/shop/image/cache/catalog/mebel/kuhnya/garnitury/stolline/catalog_11927942015012300000_1-1000x600.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'Плинтус кухонный Korner Мрамор серый №6086 3,05м L37',
    article: '127504',
    type: 'hit',
    cost: '10 999',
  },
  {
    id: 3,
    files: [
      {
        medium:
          'https://mebeltrade12.ru/upload/shop_3/4/8/5/item_48545/item_48545.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'Плинтус кухонный Korner Мрамор серый №6086 3,05м L37',
    article: '127504',
    type: 'new',
    cost: '999',
  },
  {
    id: 4,
    files: [
      {
        medium: 'https://www.tdserver.ru/upload/iblock/8d7/SET1609.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description:
      'Atira, ящик 176х470 мм., антрацит, полн. выдв-е, плавн. закр. с двойным релингом',
    article: '127504',
    type: 'sale',
    cost: '32 999',
  },
  {
    id: 5,
    files: [{ medium: '', large: '', small: '' }],
    name: 'Здесь будет название продукта',
    description: 'Подвесное кресло 2М бежевый/черный',
    article: '127504',
    type: 'hit',
    cost: '32 999',
  },
  {
    id: 6,
    files: [
      {
        medium:
          'https://nn-furnitura.ru/assets/images/products/410/dubmlechniiu1095.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'ЛДСП Дуб Млечный Свеза U10095, 3500*1750*16',
    article: '127504',
    type: 'sale',
    cost: '32 999',
  },
  {
    id: 7,
    files: [{ medium: '', large: '', small: '' }],
    name: 'Здесь будет название продукта',
    description: 'Подвесное кресло 2М бежевый/черный',
    article: '127504',
    type: 'new',
    cost: '32 999',
  },
  {
    id: 8,
    files: [
      {
        medium: 'https://cdn.etm.ru/ipro/645/small_4sb.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'Соединитель окончание 1474, 28мм',
    article: '127504',
    type: 'hit',
    cost: '32 999',
  },
  {
    id: 9,
    files: [
      {
        medium:
          'https://podvesnov.ru/wp-content/uploads/2020/10/rotang-uley-01.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'Подвесное кресло 2М бежевый/черный',
    article: '127504',
    type: 'hit',
    cost: '32 999',
  },
  {
    id: 10,
    files: [
      {
        medium:
          'https://specmebel-tver.ru/shop/image/cache/catalog/mebel/kuhnya/garnitury/stolline/catalog_11927942015012300000_1-1000x600.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'Плинтус кухонный Korner Мрамор серый №6086 3,05м L37',
    article: '127504',
    type: 'hit',
    cost: '10 999',
  },
  {
    id: 11,
    files: [
      {
        medium:
          'https://mebeltrade12.ru/upload/shop_3/4/8/5/item_48545/item_48545.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'Плинтус кухонный Korner Мрамор серый №6086 3,05м L37',
    article: '127504',
    type: 'new',
    cost: '999',
  },
  {
    id: 12,
    files: [
      {
        medium: 'https://www.tdserver.ru/upload/iblock/8d7/SET1609.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description:
      'Atira, ящик 176х470 мм., антрацит, полн. выдв-е, плавн. закр. с двойным релингом',
    article: '127504',
    type: 'sale',
    cost: '32 999',
  },
  {
    id: 13,
    files: [{ medium: '', large: '', small: '' }],
    name: 'Здесь будет название продукта',
    description: 'Подвесное кресло 2М бежевый/черный',
    article: '127504',
    type: 'hit',
    cost: '32 999',
  },
  {
    id: 14,
    files: [
      {
        medium:
          'https://nn-furnitura.ru/assets/images/products/410/dubmlechniiu1095.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'ЛДСП Дуб Млечный Свеза U10095, 3500*1750*16',
    article: '127504',
    type: 'sale',
    cost: '32 999',
  },
  {
    id: 15,
    files: [{ medium: '', large: '', small: '' }],
    name: 'Здесь будет название продукта',
    description: 'Подвесное кресло 2М бежевый/черный',
    article: '127504',
    type: 'new',
    cost: '32 999',
  },
  {
    id: 16,
    files: [
      {
        medium: 'https://cdn.etm.ru/ipro/645/small_4sb.jpg',
        large: '',
        small: '',
      },
    ],
    name: 'Здесь будет название продукта',
    description: 'Соединитель окончание 1474, 28мм',
    article: '127504',
    type: 'hit',
    cost: '32 999',
  },
];

export const promotions = [
  {
    id: 1,
    title: 'Небывалое снижение цен на AGT - скидки до 90%!',
    bgImg: offer1,
  },
  {
    id: 2,
    title: 'Посудомоечная машина в подарок!',
    bgImg: offer2,
  },
  {
    id: 3,
    title: 'Распродажа Samet - скидки до 25%!',
    bgImg: offer3,
  },
];

export const roomProducts = [
  {
    id: 1,
    title: 'Для кухни',
    items: [
      'Кухонные гарнитуры',
      'Опоры мебельные',
      'Фурнитура',
      'Столешницы',
      'Кухонные комплектующие',
    ],
    img: room1,
  },
  {
    id: 2,
    title: 'Для гостиной',
    items: ['Кухонные гарнитуры', 'Столешницы', 'Кухонные комплектующие'],
    img: room2,
  },
  {
    id: 3,
    title: 'Для ванной',
    items: ['Кухонные гарнитуры', 'Опоры мебельные', 'Фурнитура'],
    img: room3,
  },
  {
    id: 4,
    title: 'Для спальни',
    items: [
      'Кухонные гарнитуры',
      'Опоры мебельные',
      'Фурнитура',
      'Столешницы',
      'Кухонные комплектующие',
    ],
    img: room4,
  },
  {
    id: 5,
    title: 'Для  детской',
    items: ['Кухонные гарнитуры', 'Столешницы', 'Кухонные комплектующие'],
    img: room5,
  },
  {
    id: 6,
    title: 'Для прихожей',
    items: [
      'Кухонные гарнитуры',
      'Фурнитура',
      'Столешницы',
      'Кухонные комплектующие',
    ],
    img: room6,
  },
];

export const brands = [
  {
    id: 1,
    img: brand1,
  },
  {
    id: 2,
    img: brand2,
  },
  {
    id: 3,
    img: brand3,
  },
  {
    id: 4,
    img: brand4,
  },
  {
    id: 5,
    img: brand5,
  },
  {
    id: 6,
    img: brand6,
  },
  {
    id: 7,
    img: brand4,
  },
  {
    id: 8,
    img: brand3,
  },
];

export const advantages = [
  {
    id: 1,
    title: 'Высокое качество продукции',
    desc: 'Мы поставляем только проверенные плитные материалы и фурнитуру, а также проводим многоступенчатый контроль на всех этапах производства столешниц',
    icon: adv1,
    greenBlock: false,
  },
  {
    id: 2,
    title: 'Собственное производство',
    desc: 'Производство оснащено современным оборудованием, что позволяет строго соблюдать технологический процесс и выполнять изделия с точностью до миллиметра',
    icon: adv2,
    greenBlock: true,
  },
  {
    id: 3,
    title: 'Удобная форма заказа товара',
    desc: 'Мы сделали максимально простой и удобный онлайн-магазин с заботой о вас. Вы легко можете найти и заказать продукцию от ведущих производителей фурнитуры и комплектующих',
    icon: adv3,
    greenBlock: false,
  },
  {
    id: 4,
    title: 'Надежная упаковка',
    desc: 'Вся продукция надежно упаковывается, что сводит к минимуму риск механических повреждений в процессе транспортировки',
    icon: adv4,
    greenBlock: false,
  },
  {
    id: 5,
    title: 'Демократичные цены',
    desc: 'В нашем ассортименте вы найдете товары разных ценовых категорий – от среднего до премиум-класса',
    icon: adv5,
    greenBlock: false,
  },
  {
    id: 6,
    title: 'Гарантия',
    desc: 'Все реализуемые материалы и комплектующие имеют сертификацию',
    icon: adv6,
    greenBlock: true,
  },
];

export const organizations = [
  {
    id: 1,
    inn: '123456789',
    kpp: '123456789',
    address:
      '357340, Ставропольский край, город Лермонтов, ул. Волкова, д. 12а',
    ogrn: '1152651012733',
    name: 'ООО «Ромашка»',
    yurAddress:
      '357340, Ставропольский край, город Лермонтов, ул. Волкова, д. 12а',
    faqAddress: 'не заполнено',
    rasShet: '1234567890',
    bikBanka: '1234567890',
    korrSchet: '1234567890',
    bankName: 'АО «Тинькофф банк»',
  },
  {
    id: 2,
    inn: '123456789',
    kpp: '123456789',
    address:
      '357340, Ставропольский край, город Лермонтов, ул. Волкова, д. 12а',
    ogrn: '1152651012733',
    name: 'ИП Иванов Иван Иванович',
    yurAddress:
      '357340, Ставропольский край, город Лермонтов, ул. Волкова, д. 12а',
    faqAddress: 'не заполнено',
    rasShet: '1234567890',
    bikBanka: '1234567890',
    korrSchet: '1234567890',
    bankName: 'АО «Тинькофф банк»',
  },
];

export const orders = [
  {
    id: 1,
    orderNumber: '2310-1234567-12345',
    date: '12 октября 2023',
    status: 'Комплектуется',
    summ: '10 250',
    deliveryDate: '21.10.2023',
    receiveType: 'Доставка курьером',
    address: 'Россия, Москва, Вознесенский пер., д. 15',
    paymentType: 'Онлайн',
    items: [
      {
        id: 1,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Комплектуется',
      },
      {
        id: 2,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Ожидает получения',
      },
      {
        id: 3,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Выполнен',
      },
    ],
  },
  {
    id: 2,
    orderNumber: '2310-1234567-12345',
    date: '12 октября 2023',
    status: 'Ожидает получения',
    summ: '10 250',
    deliveryDate: '21.10.2023',
    receiveType: 'Доставка курьером',
    address: 'Россия, Москва, Вознесенский пер., д. 15',
    paymentType: 'Онлайн',
    items: [
      {
        id: 1,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Комплектуется',
      },
      {
        id: 2,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Ожидает получения',
      },
      {
        id: 3,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Комплектуется',
      },
    ],
  },
  {
    id: 3,
    orderNumber: '2310-1234567-12345',
    date: '12 октября 2023',
    status: 'Выполнен',
    summ: '10 250',
    deliveryDate: '21.10.2023',
    receiveType: 'Доставка курьером',
    address: 'Россия, Москва, Вознесенский пер., д. 15',
    paymentType: 'Онлайн',
    items: [
      {
        id: 1,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Ожидает получения',
      },
      {
        id: 2,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Комплектуется',
      },
      {
        id: 3,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Ожидает получения',
      },
    ],
  },
  {
    id: 4,
    orderNumber: '2310-1234567-12345',
    date: '12 октября 2023',
    status: 'Отменен',
    summ: '10 250',
    deliveryDate: '21.10.2023',
    receiveType: 'Доставка курьером',
    address: 'Россия, Москва, Вознесенский пер., д. 15',
    paymentType: 'Онлайн',
    items: [
      {
        id: 1,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Ожидает получения',
      },
      {
        id: 2,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Комплектуется',
      },
      {
        id: 3,
        name: 'Кресло мягкое Грэйс Z-14 (изумруд) на высоких ножках с подлокотниками в гостиную, офис, зону ожидания, салон красоты',
        image: chair,
        article: '127504',
        deliveryDate: '12 ноября',
        count: 1,
        cost: '19 999',
        status: 'Комплектуется',
      },
    ],
  },
];
