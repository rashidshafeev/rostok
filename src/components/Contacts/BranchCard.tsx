import { ProjectBranch } from '@/types/ServerData/PageContent/GetContactsResponse';
...
export const BranchCard = ({ branch }: BranchCardProps) => {
  return (
    <div className='w-full md:basis-[calc(33%-(20px*2)/3)] border border-colGreen rounded-xl p-4 md:p-5 flex flex-col'>
      <div className='font-semibold text-xl md:text-2xl mb-3 md:mb-4'>{branch.name}</div>
      <div className='flex flex-col justify-between grow'>
        <div className='flex flex-col gap-3 md:gap-4 mb-6 md:mb-10'>
          ...
          <div className='flex flex-col md:flex-row md:gap-2 md:items-end'> 
            <div className='text-colDarkGray text-sm md:text-base'>Режим работы</div>
            <div className='font-semibold text-base md:text-lg'>{branch.working_hours}</div>
          </div>
          <div className='flex flex-col md:flex-row md:gap-2 md:items-end'> 
            <div className='text-colDarkGray text-sm md:text-base'>Эл. почта</div>
            <div className='font-semibold text-base md:text-lg'>{branch.email}</div>
          </div>
          <div className='flex flex-col md:flex-row md:gap-2 md:items-start'> 
            ...
            <div className='font-semibold text-base md:text-lg'>{branch.address}</div>
          </div>
        </div>
