...
const PreviewGallery = ({ product } : PreviewGalleryProps) => {
  ...
  const handleMouseMove = (e) => {
    if (product?.files?.length > 1) {
      ...
      const newIndex = Math.floor((hoverPosition / width) * product?.files?.length);
  ...
  const objectFitClass = product?.files[hoveredIndex]?.stretch_image ? 'object-cover' : 'object-contain';
  ...
  return (
    ...
    onTouchEnd={onTouchEnd}>
        ...
        <img onMouseMove={handleMouseMove} onMouseLeave={() => setHoveredIndex(0)} src={displayedImage}  className={`w-full h-full ${objectFitClass}`} />
      </div>
        <div className=" flex flex-col items-center">
          {/* <div className="w-full h-full"> */}
          <img 
              
              src={displayedImage} className={`w-full h-full ${objectFitClass}`} />
          {/* </div> */}
