...
const CatProducts = () => {
   ...
   */
  const updateFiltersFromCategory = async (request: GetFiltersRequest): Promise<void> => {
    ...
    try {
      ...
      navigate(`?${CatalogQueryParamsUtil.buildQueryParams(updatedRequest)}`, { replace: true });
   ...
   */
  const updateFiltersFromUserInput = async (request: GetFiltersRequest): Promise<void> => {
   ...
   */
  const processNewFilters = (newFilters: { data: FiltersState }): FiltersState => ({
  ...
  const updateFiltersState = (newFiltersState: FiltersState): void => {
    const request = createRequest(getFiltersRequest(newFiltersState), newFiltersState.page, filters.sort);
    navigate(`?${CatalogQueryParamsUtil.buildQueryParams(request)}`, {replace: true});
    previousFilters.current = { ...previousFilters.current, ...newFiltersState };
  ...
  const handleSort = (sort: SortingParams): void => {
    ...
    const request = createRequest(getFiltersRequest(newFilters), newFilters.page, sort);
    navigate(`?${CatalogQueryParamsUtil.buildQueryParams(request)}`, {replace: true});
    getProducts(request);
  ...
  const handlePagination = (e: React.MouseEvent<unknown>, p: number): void => {
    ...
    setFilters(newFilters);
    const request = createRequest(getFiltersRequest(newFilters), p, filters.sort);
    navigate(`?${CatalogQueryParamsUtil.buildQueryParams(request)}`, {replace: true});
    getProducts(request);
    previousFilters.current = newFilters;
    scrollToTop();
  ...
  useEffect(() => {
    if (JSON.stringify(filters) !== JSON.stringify(previousFilters.current)) {
      ...
      const onlyPageChanged = 
        ...
        JSON.stringify({ ...previousFilters.current, page: previousFilters.current.page });

      if (onlyPageChanged) {
        return;
      }

      const request = createRequest(getFiltersRequest(filters), filters.page, filters.sort);
  ...
  useEffect(() => {
    ...
    if (queryParams) {
      ...
      const request = createRequest({
        ...
        min_rating: null,
        max_rating: null,
      }, 1, currentSort); // Pass the current sort explicitly
  ...
  const getFiltersRequest = (filtersState: FiltersStateWithMeta): BaseFilterParams => {
    const brands = filtersState?.basics?.brands?.reduce((acc: number[], brand) => {
