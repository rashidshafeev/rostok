...
const SaleBanner = () => {
  ...
  return (
    isSuccess && data && <div className='flex h-[90px] xs:h-[110px] md:h-[160px] xl:h-[200px] rounded-md mm:rounded-xl overflow-hidden'>
      <div className='w-1/2 md:w-2/5  flex lg:justify-center items-center px-3 sm:px-5'
      style={{
        backgroundColor: data?.staticBaner?.background_color
      }}>
        <h1 className={`xs:text-lg sm:text-xl mm:text-3xl md:text-4xl lg:text-5xl xl:text-[54px] uppercase xl:leading-[58px] font-[900] text-[] xl:pl-10 pr-5 mm:pr-24 lg:pr-0`}
        style={{
      ...
      <div className='w-1/2 md:w-3/5 relative'>
        ...
        <img className='w-full h-full' src={data?.staticBaner?.picture?.medium} alt='*' />
      </div>
    </div>
  );
};
