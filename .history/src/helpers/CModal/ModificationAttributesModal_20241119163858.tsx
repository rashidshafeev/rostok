...
// Add this new component above ModalContent
const AttributesList = memo(({ 
  ...
  return (
    <>
      {Object.keys(attributesList).map((attr) => (
        <div key={`${attr}-${attributesList[attr].values.find(v => v.current)?.value}`} className="mb-4">
  ...
  return JSON.stringify(prevProps.attributesList) === JSON.stringify(nextProps.attributesList);
});
